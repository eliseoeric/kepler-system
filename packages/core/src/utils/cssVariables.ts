 ...
 */
export function tailwindCSSVar(
  tokenName: string,
  version?: string | number,
): string {
  if (version) {
...
export function createVersionedStyles(version: string | number) {
  return {
    ...
    fontWeight: (token: string) =>
      getCSSVariable(`font-weight-${token}`, version),
    borderRadius: (token: string) => getCSSVariable(`radii-${token}`, version),
    motion: {
      duration: (token: string) =>
        getCSSVariable(`motion-duration-${token}`, version),
      easing: (token: string) =>
        getCSSVariable(`motion-easing-${token}`, version),
    },
  };
}
